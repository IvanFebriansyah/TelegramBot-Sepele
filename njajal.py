...
def start(bot, update):
    update.message.reply_text("Assalamu'alaikum {}!"
        ...
        "\n▀▀█▄created by Iv4n▄▄▄▄▄▄▄▄▄█▀▀"
        "\n"
		"\nSelamat datang di Bot cilik."
		"\nBot Cilik akan membantumu untuk menampilkan jadwal shalat, ayat Al-Quran dan do'a sehari-hari."
...
def cari(bot, update, args):
    try:
        kata = args[0].lower()
        r = \
            requests.get('http://api.banghasan.com/quran/format/json/cari/{}/bahasa/id/mulai/0/limit/100'.format(kata))
...
def catatan(bot, update, args):
    try:
        ...
        if no in range(1, 1611):
            r = \
                requests.get('http://api.fathimah.ga/quran/format/json/catatan/{}'.format(no))
            data = json.loads(r.content)
...
Sumber: Daily essential duas oleh http://www.duaandazkar.com/""".format(name,
                    ref, pronun, trans, hadith))
        else:
...
def alarm(bot, job):
    ...
    bot.send_message(job.context,
                     text="Assalamu'alaikum! Ayat pilihan hari ini adalah QS.{}:{}".format(s,
                     a))
    ...
    if match is not None:
        ...
        akhir = int(akhir) - int(awal) + 1
        r = \
            requests.get('http://api.fathimah.ga/quran/format/json/surat/{}/ayat/{}'.format(s,
                         a))
        data = r.content
        data = json.loads(data)
        for i in range(akhir):
...
def setdaily(
    ...
    try:
        ...
        job = job_queue.run_repeating(alarm, 86400, first=15,
                context=chat_id)
        chat_data['job'] = job
...
def main():
    updater = Updater('1616228902:AAEA0rnomuuHDpoGqxODZKvKczTAaV65KMs')
